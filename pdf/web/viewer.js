  ...
  /******/ var __webpack_modules__ = [
          ...
          return loadingTask.promise.then(
            ...
            (reason) => {
              ...
              return this.l10n.get(key).then((msg) => {
                ...
                document.querySelector('#pdfError').classList.remove('page--error__hidden')
    ...
    /***/ (__unused_webpack_module, exports, __webpack_require__) => {
      ...
      const DIACRITICS_EXCEPTION = new Set([
        0x3099, 0x309a, 0x094d, 0x09cd, 0x0a4d, 0x0acd, 0x0b4d, 0x0bcd, 0x0c4d, 0x0ccd, 0x0d3b,
        0x0d3c, 0x0d4d, 0x0dca, 0x0e3a, 0x0eba, 0x0f84, 0x1039, 0x103a, 0x1714, 0x1734, 0x17d2,
        0x1a60, 0x1b44, 0x1baa, 0x1bab, 0x1bf2, 0x1bf3, 0x2d7f, 0xa806, 0xa82c, 0xa8c4, 0xa953,
        0xa9c0, 0xaaf6, 0xabed, 0x0c56, 0x0f71, 0x0f72, 0x0f7a, 0x0f7b, 0x0f7c, 0x0f7d, 0x0f80,
        0x0f74,
      ])
      ...
      class PDFHistory {
        ...
        push({ namedDest = null, explicitDest, pageNumber }) {
          ...
          if (namedDest && typeof namedDest !== 'string') {
            console.error(
              'PDFHistory.push: ' + `"${namedDest}" is not a valid namedDest parameter.`,
            )
            return
          } else if (!Array.isArray(explicitDest)) {
            console.error(
              'PDFHistory.push: ' + `"${explicitDest}" is not a valid explicitDest parameter.`,
            )
            return
          } else if (!this._isValidPage(pageNumber)) {
            if (pageNumber !== null || this._destination) {
              console.error(
                'PDFHistory.push: ' + `"${pageNumber}" is not a valid pageNumber parameter.`,
              )
              return
            }
          }
      ...
      class PDFPresentationMode {
        ...
        #mouseWheel(evt) {
          ...
          if (Math.abs(this.mouseScrollDelta) >= PAGE_SWITCH_THRESHOLD) {
            ...
            this.#resetMouseScrollState()
            const success =
              totalDelta > 0 ? this.pdfViewer.previousPage() : this.pdfViewer.nextPage()
            if (success) {
      ...
      class PDFViewer {
        ...
        #initializePermissions(permissions) {
          ...
          if (
            ...
            this.#textLayerMode === _ui_utils.TextLayerMode.ENABLE
          ) {
        ...
        #copyCallback(textLayerMode, event) {
          ...
          if (anchorNode && focusNode && selection.containsNode(this.#hiddenCopyElement)) {
            if (
              ...
              textLayerMode === _ui_utils.TextLayerMode.ENABLE_PERMISSIONS
            ) {
              event.preventDefault()
              event.stopPropagation()
              return
        ...
        setDocument(pdfDocument) {
          ...
          Promise.all([firstPagePromise, permissionsPromise])
            .then(([firstPdfPage, permissions]) => {
              ...
              const viewerElement =
                this._scrollMode === _ui_utils.ScrollMode.PAGE ? null : this.viewer
          ...
          div.addEventListener('copy', (event) => {
            if (!this.#enablePermissions) {
              ...
              event.clipboardData.setData(
                'text/plain',
                (0, _ui_utils.removeNullCharacters)(
                  (0, _pdfjsLib.normalizeUnicode)(selection.toString()),
                ),
              )
            }
            event.preventDefault()
